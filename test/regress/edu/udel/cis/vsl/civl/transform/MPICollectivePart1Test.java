...
public class MPICollectivePart1Test {
	...
	public void bcast_bad() {
		assertFalse(ui.run("verify -input_mpi_nprocs=6 ", ConstantsTest.QUIET,
	...
	public void bcast_good() {
		assertFalse(ui.run("verify -DFASSERT -input_mpi_nprocs=3 ",
				ConstantsTest.QUIET, NO_PRINTF, filename("bcast_good.c")));
		assertTrue(ui.run("verify -input_mpi_nprocs=3", ConstantsTest.QUIET,
				NO_PRINTF, filename("bcast_good.c")));
	}
	...
	public void BcastReduce_bad() {
		assertFalse(ui.run("verify -input_mpi_nprocs=10 ", ConstantsTest.QUIET,
	...
	public void gather_type() {
		assertFalse(ui.run("verify -DTYPE -input_mpi_nprocs=6 ",
				ConstantsTest.QUIET, filename("gather.c")));
	}
	...
	public void scatter_type() {
		assertFalse(ui.run("verify -DTYPE -input_mpi_nprocs=6 ",
				ConstantsTest.QUIET, filename("scatter.c")));
	}
