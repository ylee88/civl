...
import edu.udel.cis.vsl.civl.transform.analysis.common.CommonFlowInsensePointsToAnalyzer;
...
public class OpenMPSimplifierWorker2 extends BaseWorker {
	...
	// TODO: make this field local
	private Function function;

	private FlowInsensePointsToAnalyzer pointsToAnalyzer = null;
	...
	public AST transform(AST unit) throws SyntaxException {
		...
		////////////////////
		InvocationGraphFactory igFactory = SimplePointsToAnalysis
				.newInvocationGraphFactory();
		pointsToAnalyzer = new CommonFlowInsensePointsToAnalyzer(unit,
				SimplePointsToAnalysis.newAssignmentFactory(igFactory),
				igFactory);
		////////////////////
	...
	private void collectAssignRefExprs(ASTNode node) {
		if (node instanceof VariableDeclarationNode) {
			...
			if (lhs instanceof IdentifierExpressionNode) {
				...
				if (((OperatorNode) lhs).getOperator() == Operator.SUBSCRIPT) {
				...
				} else if (((OperatorNode) lhs)
						.getOperator() == Operator.DEREFERENCE) {
					OpenMPSimplifierWorkerComponent.collectDereference(
							privateIDs, loopPrivateIDs, locallyDecls, writeVars,
							(OperatorNode) lhs, function, pointsToAnalyzer);
				}
			} else {
	...
	private void collectRHSRefExprs(ASTNode node) {
		if (node instanceof IdentifierExpressionNode) {
			...
			// TODO: what about index expressions ?
		} else if (node != null) {
			// BUG: can get here with null values in parallelfor.c example
			...
			if (node instanceof OperatorNode && ((OperatorNode) node)
					.getOperator() == Operator.DEREFERENCE) {
				OpenMPSimplifierWorkerComponent.collectDereference(privateIDs,
						loopPrivateIDs, Arrays.asList(), readVars,
						(OperatorNode) node, function, pointsToAnalyzer);
			}
		}
	}
