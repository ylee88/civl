...
public class ContractTransformerWorker extends BaseWorker {
	...
	private class ParsedContractBlock {
		...
		private boolean complete() {
			...
			for (ConditionalClauses behav : behaviors) {
				if (!(behav.requiresSet.isEmpty() && behav.ensuresSet.isEmpty() && behav.waitsforSet
						.isEmpty()))
	...
	private FunctionDefinitionNode mainFunction(
		...
		callDriver = nodeFactory
				.newExpressionStatementNode(nodeFactory.newFunctionCallNode(
						...
						identifierExpression(DRIVER_PREFIX
								+ targetFunc.getName()), Arrays.asList(), null));
		if (hasMPI) {
		...
		FunctionTypeNode mainType = nodeFactory.newFunctionTypeNode(
				this.newSource("type of the main function",
						CivlcTokenConstant.TYPE), this
						.basicType(BasicTypeKind.INT), mainFormals, true);

		return nodeFactory.newFunctionDefinitionNode(this.newSource(
	...
	private FunctionDefinitionNode transformContractedFunction(
		...
		Source mpiCommRankSource = newSource(
				"int " + MPI_COMM_RANK_CONST + ";",
				CivlcTokenConstant.DECLARATION);
		Source mpiCommSizeSource = newSource(
				"int " + MPI_COMM_SIZE_CONST + ";",
				CivlcTokenConstant.DECLARATION);
		...
		bodyItems.add(nodeFactory.newVariableDeclarationNode(contractSource,
				identifier(RESULT), funcDecl.getTypeNode().getReturnType()
						.copy()));
	...
	private FunctionDefinitionNode transformTargetFunction(
		...
		List<ParsedContractBlock> parsedContractBlocks = parseFunctionContracts(funcDefi
				.getContract());
		...
		for (VariableDeclarationNode param : funcTypeNode.getParameters())
			funcParamIdentfiers.add(identifierExpression(param.getIdentifier()
					.name()));
		targetCall = nodeFactory.newFunctionCallNode(driverSource,
				funcIdentifier.copy(), funcParamIdentfiers, null);
		bodyItems.add(nodeFactory.newVariableDeclarationNode(contractSource,
				identifier(RESULT), funcDefi.getTypeNode().getReturnType()
						.copy(), targetCall));
		...
		funcTypeNode = nodeFactory.newFunctionTypeNode(
				funcTypeNode.getSource(), funcTypeNode.getReturnType().copy(),
				nodeFactory.newSequenceNode(funcTypeNode.getParameters()
						.getSource(), "contract_driver_parameters", Arrays
						.asList()), funcTypeNode.hasIdentifierList());
	...
	private List<BlockItemNode> transformCoRequirements4NT(
		...
		coAssertsComponents.add(createAssertion(commEmptyInAndOut(mpiComm,
				mpiBlock.pattern)));
		...
		stmt = nodeFactory.newWithNode(stmt.getSource(),
				identifierExpression(collateStateDecl.getName()), stmt, false);
	...
	private List<BlockItemNode> transformCollectiveEnsures4Target(
		...
		for (ConditionalClauses condClauses : mpiBlock.getConditionalClauses()) {
			...
			for (ExpressionNode ensures : condClauses.getEnsures(nodeFactory))
				coAssertStmtComponents.add(translateConditionalPredicates(
	...
	private StatementNode translateEnsurance2Inference(
		...
		stmt = nodeFactory.newWithNode(collateStateRef.getSource(),
				collateStateRef.copy(), stmt, false);
		if (hasGuard) {
	 ...
	 * @return
	 */
	private ExpressionNode createMPIContractEntered(
			List<ExpressionNode> ranges, ExpressionNode mpiComm) {
		Source source = newSource(MPI_CONTRACT_ENTERED + "(" + mpiComm
				+ ", ...)", CivlcTokenConstant.CALL);
	...
	private StatementNode createMPIInitCall() throws SyntaxException {
		...
		TypeNode ptr2Void = nodeFactory.newPointerTypeNode(
				newSource("(void *)", CivlcTokenConstant.TYPE), nodeFactory
						.newVoidTypeNode(newSource("void",
								CivlcTokenConstant.TYPE)));
		CastNode nullPtr = nodeFactory
				.newCastNode(newSource("(void *)0", CivlcTokenConstant.CAST),
						ptr2Void, zero);
		return nodeFactory.newExpressionStatementNode(nodeFactory
				.newFunctionCallNode(
						newSource("MPI_Init(NULL, NULL);",
								CivlcTokenConstant.CALL),
						identifierExpression(MPI_INIT_CALL), Arrays.asList(
								nullPtr, nullPtr.copy()), null));
	}
	...
	private VariableDeclarationNode createCollateStateDeclaration(
		...
		Source source = newSource(COLLATE_STATE + " "
				+ DRIVER_COLLATE_STATE_VAR_PRE + (varNameCounter) + " = "
				+ MPI_SNAPSHOT, CivlcTokenConstant.DECLARATION);
		InitializerNode initializer = createMPISnapshotCall(mpiComm.copy());
		TypeNode collateStateTypeName = nodeFactory.newTypedefNameNode(
	...
	private StatementNode createHavocCall(ExpressionNode var) {
		Source source = newSource(HAVOC + "(" + var.prettyRepresentation()
				+ ");", CivlcTokenConstant.CALL);
	 ...
	 *         call
	 */
	private ExpressionNode createCollateArrivedCall(
			List<ExpressionNode> ranges, ExpressionNode stateRef) {
		...
		for (ExpressionNode range : ranges)
			arguments.add(range.copy());
		return nodeFactory.newFunctionCallNode(source, arrivedFuncId,
				arguments, null);
	}
	...
	private ExpressionNode commEmptyInAndOut(ExpressionNode mpiComm,
		...
		arg = colKind == MPICollectiveKind.P2P ? nodeFactory.newDotNode(
				mpiComm.getSource(), mpiComm.copy(), identifier(P2P))
	...
	private List<FunctionDeclarationNode> createDeclarationsForUsedFunctions() {
		...
		TypeNode boolTypeNode = nodeFactory.newBasicTypeNode(boolTypeSource,
				BasicTypeKind.BOOL);
		TypeNode mpiCommTypeNode = nodeFactory.newTypedefNameNode(
				identifier(MPI_COMM_TYPE), null);
		TypeNode collateStateTypeNode = nodeFactory.newTypedefNameNode(
				identifier(COLLATE_STATE), null);
		...
		TypeNode ptr2ptr2ptr2charNode = nodeFactory.newPointerTypeNode(
				ptr2charSource, nodeFactory.newPointerTypeNode(ptr2charSource,
		...
		results.add(functionDeclarationMaker(MPI_COMM_RANK_CALL, formals,
				intTypeNode.copy(), false, "int MPI_Comm_rank(MPI_Comm, int *)"));
		// MPI_Comm_size:
		...
		results.add(functionDeclarationMaker(MPI_COMM_SIZE_CALL, formals,
				intTypeNode.copy(), false, "int MPI_Comm_size(MPI_Comm, int *)"));
		// MPI_Init:
		...
		decl = functionDeclarationMaker(COLLATE_COMPLETE, formals,
				boolTypeNode, false, "_Bool $collate_complete($collate_state)");
		decl.setSystemFunctionSpecifier(true);
	...
	private List<ParsedContractBlock> parseFunctionContracts(
		...
		ParsedContractBlock localMainBlock = new ParsedContractBlock(null,
				null, contracts.getSource());
	...
	private void parseClausesInBehavior(SequenceNode<ContractNode> contracts,
		...
		for (ContractNode contract : contracts)
			if (contract.contractKind() == ContractKind.ASSUMES) {
				...
				assumptions = assumptions == null ? assumes : nodeFactory
						.newOperatorNode(assumes.getSource(), Operator.LAND,
								assumptions, assumes);
			}
		...
		for (ContractNode contract : contracts) {
			...
			case ENSURES:
				...
				break;
			case WAITSFOR:
				condClauses.addWaitsfor(((WaitsforNode) contract)
						.getArguments());
				break;
			default:
				// do nothing.
