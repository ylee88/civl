...
public class ContractTransformerWorker extends BaseWorker {
		...
		if (localBlock != null)
			for (ConditionalClauses condClause : localBlock.getConditionalClauses())
				for (ExpressionNode requires : condClause.getRequires(nodeFactory))
					bodyItems.add(translateConditionalPredicates(false, condClause.condition, requires));
		...
		if (localBlock != null) {
			for (ConditionalClauses condClauses : localBlock.getConditionalClauses())
				for (ExpressionNode ensures : condClauses.getEnsures(nodeFactory))
					bodyItems.add(translateConditionalPredicates(true, condClauses.condition, ensures));
		}
	...
	private FunctionDefinitionNode transformTargetFunction(FunctionDefinitionNode funcDefi) {
		...
		if (localBlock != null)
			for (ConditionalClauses requires : localBlock.getConditionalClauses())
				for (ExpressionNode pred : requires.getRequires(nodeFactory))
					bodyItems.add(translateConditionalPredicates(true, requires.condition, pred));
		...
		for (ParsedContractBlock mpiBlock : parsedContractBlocks)
			for (Pair<ExpressionNode, List<ExpressionNode>> condWaitsforArgs : mpiBlock.getConditionalWaitsfors())
				bodyItems.add(checkWaitsfors(condWaitsforArgs.left, condWaitsforArgs.right, mpiBlock.mpiComm,
						mpiBlock.source));
		...
		if (localBlock != null)
			for (ConditionalClauses ensures : localBlock.getConditionalClauses())
				for (ExpressionNode pred : ensures.getEnsures(nodeFactory))
					bodyItems.add(translateConditionalPredicates(false, ensures.condition, pred));
	...
	private List<BlockItemNode> transformCoRequirements4Target(ParsedContractBlock mpiBlock, Source mpiCommRankSource,
		...
		VariableDeclarationNode collateStateDecl = createCollateStateDeclaration(DRIVER_COLLATE_STATE_VAR_PRE, mpiComm);
		...
		for (ConditionalClauses condClauses : mpiBlock.getConditionalClauses())
			for (ExpressionNode requires : condClauses.getRequires(nodeFactory))
				coAssumesComponents.add(translateConditionalPredicates(true, condClauses.condition, requires));
		...
		coAssumeStmt = nodeFactory.newWithNode(coAssumeStmt.getSource(),
				identifierExpression(collateStateDecl.getName()), coAssumeStmt, false);
	 ...
	 *         statements
	 */
	private List<BlockItemNode> transformCoRequirements4NT(ParsedContractBlock mpiBlock) {
		ExpressionNode mpiComm = mpiBlock.mpiComm;
		VariableDeclarationNode collateStateDecl = createCollateStateDeclaration(DRIVER_COLLATE_STATE_VAR, mpiComm);
		...
		for (ConditionalClauses condClauses : mpiBlock.getConditionalClauses())
			for (ExpressionNode requires : condClauses.getRequires(nodeFactory))
				coAssertsComponents.add(translateConditionalPredicates(false, condClauses.condition, requires));
		...
		stmt = nodeFactory.newWithNode(stmt.getSource(), identifierExpression(collateStateDecl.getName()), stmt, false);
	 ...
	 * @return
	 */
	private List<BlockItemNode> transformCollectiveEnsures4Target(ParsedContractBlock mpiBlock) {
		ExpressionNode mpiComm = mpiBlock.mpiComm;
		VariableDeclarationNode collateStateDecl = createCollateStateDeclaration(DRIVER_COLLATE_STATE_VAR_POST,
				mpiComm);
		StatementNode stmt;
		List<BlockItemNode> bodyItems = new LinkedList<>();
		...
		for (ConditionalClauses condClauses : mpiBlock.getConditionalClauses()) {
			...
			for (ExpressionNode ensures : condClauses.getEnsures(nodeFactory))
				coAssertStmtComponents.add(translateConditionalPredicates(false, condClauses.condition, ensures));
			stmt = nodeFactory.newCompoundStatementNode(mpiBlock.source, coAssertStmtComponents);
			withStmt = nodeFactory.newWithNode(stmt.getSource(), identifierExpression(collateStateDecl.getName()), stmt,
					false);
	...
	private List<BlockItemNode> transformCoEnsurances4NT(ParsedContractBlock mpiBlock) {
		StatementNode stmt;
		List<BlockItemNode> bodyItems = new LinkedList<>();
		VariableDeclarationNode collateStateDecl = createCollateStateDeclaration(DRIVER_COLLATE_STATE_VAR_PRE,
				mpiBlock.mpiComm);
	 ...
	 */
	private StatementNode translateConditionalPredicates(boolean isAssume, ExpressionNode cond, ExpressionNode preds) {
		StatementNode stmt = isAssume ? createAssumption(preds) : createAssertion(preds);
	...
	private StatementNode translateEnsurance2Inference(ExpressionNode collateStateRef, ExpressionNode condition,
		...
		stmt = nodeFactory.newWithNode(collateStateRef.getSource(), collateStateRef.copy(), stmt, false);
		if (hasGuard) {
	...
	private VariableDeclarationNode createCollateStateDeclaration(String varName, ExpressionNode mpiComm) {
		Source source = newSource(
				COLLATE_STATE + " " + DRIVER_COLLATE_STATE_VAR_PRE + (varNameCounter) + " = " + MPI_SNAPSHOT,
		...
		String generatedVarName = identifierPrefix + varName + (varNameCounter++);
	...
	private List<FunctionDeclarationNode> createDeclarationsForUsedFunctions() {
		...
		TypeNode intTypeNode = nodeFactory.newBasicTypeNode(intTypeSource, BasicTypeKind.INT);
		TypeNode boolTypeNode = nodeFactory.newBasicTypeNode(boolTypeSource, BasicTypeKind.BOOL);
		TypeNode mpiCommTypeNode = nodeFactory.newTypedefNameNode(identifier(MPI_COMM_TYPE), null);
		TypeNode collateStateTypeNode = nodeFactory.newTypedefNameNode(identifier(COLLATE_STATE), null);
		...
		TypeNode ptr2voidNode = nodeFactory.newPointerTypeNode(ptr2voidSource, nodeFactory.newVoidTypeNode(voidSource));
		TypeNode ptr2intNode = nodeFactory.newPointerTypeNode(ptr2intSource, intTypeNode);
		TypeNode ptr2ptr2ptr2charNode = nodeFactory.newPointerTypeNode(ptr2charSource,
				nodeFactory.newPointerTypeNode(ptr2charSource, nodeFactory.newPointerTypeNode(ptr2charSource,
						nodeFactory.newBasicTypeNode(ptr2charSource, BasicTypeKind.CHAR))));

		// MPI_Comm_rank:
		formals = Arrays.asList(mpiCommTypeNode, ptr2intNode);
		results.add(functionDeclarationMaker(MPI_COMM_RANK_CALL, formals, intTypeNode.copy(), false,
				"int MPI_Comm_rank(MPI_Comm, int *)"));
		// MPI_Comm_size:
		formals = Arrays.asList(mpiCommTypeNode.copy(), ptr2intNode.copy());
		results.add(functionDeclarationMaker(MPI_COMM_SIZE_CALL, formals, intTypeNode.copy(), false,
				"int MPI_Comm_size(MPI_Comm, int *)"));
		// MPI_Init:
		formals = Arrays.asList(ptr2intNode.copy(), ptr2ptr2ptr2charNode);
		results.add(functionDeclarationMaker(MPI_INIT_CALL, formals, intTypeNode.copy(), false,
				"int MPI_Init(int *, char ***)"));
		// MPI_Finalize:
		formals = Arrays.asList();
		results.add(
				functionDeclarationMaker(MPI_FINALIZE_CALL, formals, intTypeNode.copy(), false, "int MPI_Finalize()"));
		// $havoc:
		formals = Arrays.asList(ptr2voidNode.copy());
		decl = functionDeclarationMaker(HAVOC, formals, nodeFactory.newVoidTypeNode(voidSource), false,
				"void $havoc(void *)");
		decl.setSystemFunctionSpecifier(true);
		results.add(decl);
		// $mpi_snapshot:
		formals = Arrays.asList(mpiCommTypeNode.copy());
		results.add(functionDeclarationMaker(MPI_SNAPSHOT, formals, collateStateTypeNode, false,
				"$collate_state $mpi_snapshot(MPI_Comm )"));
		// $mpi_contract_enters:
		formals = Arrays.asList(mpiCommTypeNode.copy());
		results.add(functionDeclarationMaker(MPI_CONTRACT_ENTERS, formals, intTypeNode.copy(), false,
				"int $mpi_contract_enters(MPI_Comm )"));
		// $mpi_contract_entered:
		formals = Arrays.asList(mpiCommTypeNode.copy(), rangeTypeNode);
		results.add(functionDeclarationMaker(MPI_CONTRACT_ENTERED, formals, intTypeNode.copy(), true,
				"int $mpi_contract_entered(MPI_Comm, $range)"));
		// $collate_complete:
		formals = Arrays.asList(collateStateTypeNode.copy());
		decl = functionDeclarationMaker(COLLATE_COMPLETE, formals, boolTypeNode, false,
				"_Bool $collate_complete($collate_state)");
		decl.setSystemFunctionSpecifier(true);
		decl.setPureFunctionSpecifier(true);
		results.add(decl);
		// $collate_arrived:
		formals = Arrays.asList(collateStateTypeNode.copy());
		decl = functionDeclarationMaker(COLLATE_ARRIVED, formals, boolTypeNode.copy(), true,
	 ...
	 * @return
	 */
	private FunctionDeclarationNode functionDeclarationMaker(String functionName, List<TypeNode> formalTypes,
		...
		for (TypeNode formalType : formalTypes) {
			varDecl.add(nodeFactory.newVariableDeclarationNode(newSource("type id", CivlcTokenConstant.DECLARATION),
					identifier(commPrefix + (idcounter++)), formalType));
		}
		varDeclSeq = nodeFactory.newSequenceNode(newSource("formal types", CivlcTokenConstant.SEQUENCE), "formal_types",
				varDecl);
		funcType = nodeFactory.newFunctionTypeNode(newSource(source, CivlcTokenConstant.TYPE), returnType, varDeclSeq,
				false);
		funcType.setVariableArgs(hasVariableArguments);
		return nodeFactory.newFunctionDeclarationNode(newSource(source, CivlcTokenConstant.DECLARATION),
