...
public class LibstringExecutor extends BaseLibraryExecutor implements
	...
	private State executeWork(State state, int pid, CallOrSpawnStatement call)
		...
		switch (name.name()) {
		case "strcpy":
			state = executeStrcpy(state, pid, process, lhs, arguments,
	...
	private State executeStrcpy(State state, int pid, String process,
			LHSExpression lhs, Expression[] arguments,
			SymbolicExpression[] argumentValues, CIVLSource source)
			throws UnsatisfiablePathConditionException {
		Evaluation eval = evaluator.dereference(arguments[1].getSource(),
				state, process, argumentValues[1], false);
		...
		if (lhs != null)
			state = primaryExecutor.assign(state, pid, process, lhs, value);
		return state;
	}
