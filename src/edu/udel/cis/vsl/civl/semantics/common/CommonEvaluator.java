...
public class CommonEvaluator implements Evaluator {
		...
		if (!pointer.type().equals(this.pointerType)) {
			...
			if (sid == ModelConfiguration.DYNAMIC_REMOVED_SCOPE) {
				...
				if (sid == ModelConfiguration.DYNAMIC_CONSTANT_SCOPE) {
					variableValue = this.modelFactory.model().staticConstantScope().variable(vid).constantValue();
				} else {
	...
	protected Evaluation evaluateAddressOf(State state, int pid, AddressOfExpression expression)
		...
		if (expression.isFieldOffset()) {
			...
			SymbolicExpression typeValue = this.symbolicUtil.expressionOfType(structType,
	 ...
	 * @throws UnsatisfiablePathConditionException
	 */
	protected Evaluation evaluateArrayLiteral(State state, int pid, ArrayLiteralExpression expression)
			throws UnsatisfiablePathConditionException {
	...
	protected Evaluation evaluateRemoteOperation(State state, int pid, BinaryExpression expression)
			throws UnsatisfiablePathConditionException {
		return new QuantifiedExpressionEvaluator(modelFactory, stateFactory, libLoader, libExeLoader, symbolicUtil,
				symbolicAnalyzer, memUnitFactory, errorLogger, civlConfig).evaluateRemoteOperation(state, pid,
						expression);
	}
	...
	protected Evaluation evaluateInitialValue(State state, int pid, InitialValueExpression expression)
		...
		if (type.isArrayType())
			if (((CIVLArrayType) type).isComplete())
				state = checkArrayExtentDeclaration(state, pid, (CIVLCompleteArrayType) type, expression.getSource());
	 ...
	 * @throws UnsatisfiablePathConditionException
	 */
	private State checkArrayExtentDeclaration(State state, int pid, CIVLCompleteArrayType arrayType, CIVLSource source)
			throws UnsatisfiablePathConditionException {
		...
		state = eval.state;
		validArrayLength = universe.lessThan(zero, (NumericExpression) eval.value);
		...
		if (resultType != ResultType.YES) {
			...
			state = errorLogger.logError(source, state, process, symbolicAnalyzer.stateInformation(state),
					validArrayLength, resultType, ErrorKind.ARRAY_DECLARATION,
		...
		if (arrayType.elementType().isArrayType()) {
			...
			if (elementType.isComplete())
				return checkArrayExtentDeclaration(state, pid, (CIVLCompleteArrayType) elementType, source);
		}
		return state;
	}
	...
	protected Evaluation evaluateNumericOperations(State state, int pid, String process, BinaryExpression expression)
		...
		case MODULO: {
			...
			if (this.civlConfig.svcomp()) {
				BooleanExpression leftPositive = universe.lessThan(zero, (NumericExpression) left);
				ResultType resultType = universe.reasoner(assumption).valid(leftPositive).getResultType();

				if (resultType != ResultType.YES) {
					left = universe.add(universe.modulo((NumericExpression) left, denominator), denominator);
				}
			} else {
	 ...
	 * @throws UnsatisfiablePathConditionException
	 */
	protected Evaluation evaluateStructOrUnionLiteral(State state, int pid, StructOrUnionLiteralExpression expression)
			throws UnsatisfiablePathConditionException {
	...
	protected Evaluation evaluateGuardofSystemFunction(CIVLSource source, State state, int pid, String library,
		...
		if (function.functionContract() != null) {
			...
			if (guard != null) {
				...
				state = stateFactory.pushCallStack(state, pid, function, argumentValues);
	...
	public Evaluation evaluate(State state, int pid, Expression expression, boolean checkUndefinedValue)
		...
		case FUNC_CALL:
			...
			break;
		case STATE_REF:
			result = new Evaluation(state, modelFactory.stateValue(((StateExpression) expression).id()));
			break;
		...
		case ORIGINAL: {
			result = evaluateOriginalExpression(state, pid, (OriginalExpression) expression);
			break;
		}
		case MEMORY_UNIT:
		case NULL_LITERAL:
	...
	protected Evaluation evaluateOriginalExpression(State state, int pid, OriginalExpression original)
			throws UnsatisfiablePathConditionException {
		return new QuantifiedExpressionEvaluator(modelFactory, stateFactory, libLoader, libExeLoader, symbolicUtil,
				symbolicAnalyzer, memUnitFactory, errorLogger, civlConfig).evaluateOriginalExpression(state, pid,
						original);
	}

	protected Evaluation evaluateValueAtExpression(State state, int pid, ValueAtExpression valueAt)
			throws UnsatisfiablePathConditionException {
		return new QuantifiedExpressionEvaluator(modelFactory, stateFactory, libLoader, libExeLoader, symbolicUtil,
				symbolicAnalyzer, memUnitFactory, errorLogger, civlConfig).evaluateValueAtExpression(state, pid,
						valueAt);
	}
	...
	protected Evaluation evaluateQuantifiedExpression(State state, int pid, QuantifiedExpression expression)
			throws UnsatisfiablePathConditionException {
		return new QuantifiedExpressionEvaluator(modelFactory, stateFactory, libLoader, libExeLoader, symbolicUtil,
				symbolicAnalyzer, memUnitFactory, errorLogger, civlConfig).evaluateQuantifiedExpression(state, pid,
						(QuantifiedExpression) expression);
	}
	...
	protected Evaluation evaluateExtendedQuantifiedExpression(State state, int pid,
		...
		if (function.expressionKind() == ExpressionKind.LAMBDA) {
			...
			state = eval.state;
			lambda = eval.value;
			eval.value = applyLambda4ExtendedQuantfication(state, pid, extQuant.getSource(), quant, lambda, low, high);
		} else {
			throw new CIVLUnimplementedFeatureException("using non-lambda functions in " + quant + " expressions",
					extQuant);
		}
		return eval;
	}

	private SymbolicExpression applyLambda4ExtendedQuantfication(State state, int pid, CIVLSource source,
			ExtendedQuantifier quant, SymbolicExpression lambda, NumericExpression low, NumericExpression high)
			throws UnsatisfiablePathConditionException {
		...
		if (reasonResult == ResultType.YES) {
			...
			do {
				...
				switch (quant) {
				case SUM:
					result = result == null ? (NumericExpression) current
							: universe.add(result, (NumericExpression) current);
					break;
				case NUMOF: {
					if (reasoner.isValid((BooleanExpression) current)) {
						result = result == null ? one : universe.add(result, one);
					} else if (!reasoner.isValid(universe.not((BooleanExpression) current))) {
						errorLogger.logSimpleError(source, state, state.getProcessState(pid).name(),
				...
				case PROD:
					result = result == null ? (NumericExpression) current
							: universe.multiply(result, (NumericExpression) current);
					break;
				default:
					throw new CIVLUnimplementedFeatureException("evaluating extended quantification " + quant, source);
				}
			...
			if (reasonResult == ResultType.YES) {
				...
				default:
					errorLogger.logSimpleError(source, state, state.getProcessState(pid).name(),
							symbolicAnalyzer.stateInformation(state), ErrorKind.OTHER, "undefined input for " + quant);
					throw new UnsatisfiablePathConditionException();
				}
			} else {
				errorLogger.logSimpleError(source, state, state.getProcessState(pid).name(),
						...
						"unable to decide the LE/GT relation between the lower bound "
								+ symbolicAnalyzer.symbolicExpressionToString(source, state, null, low)
								+ " and the upper bound "
								+ symbolicAnalyzer.symbolicExpressionToString(source, state, null, high) + quant);
				throw new UnsatisfiablePathConditionException();
			}
		}
		return result;
	...
	public Evaluation dereference(CIVLSource source, State state, String process, Expression pointerExpr,
			SymbolicExpression pointer, boolean checkOutput, boolean strict)
			throws UnsatisfiablePathConditionException {
		return dereference(source, state, process, pointerExpr, pointer, checkOutput, false, strict);
	}
	...
	public Triple<State, StringBuffer, Boolean> getString(CIVLSource source, State state, String process,
		...
		if (operator == SymbolicOperator.ARRAY) {// string literal
			...
			if (ref instanceof ArrayElementReference) {
				...
				eval = dereference(source, state, process, null, pointerCharArray, false, true);
				state = eval.state;
				...
				eval = dereference(source, state, process, charPointerExpr, charPointer, false, true);
				state = eval.state;
	...
	public Evaluation getStringExpression(State state, String process, CIVLSource source,
		...
		if (symRef.isArrayElementReference()) {
			...
			Evaluation eval = this.dereference(source, state, process, null, arrayReference, false, true);
			int index;
	...
	public Evaluation pointerAdd(State state, int pid, String process, BinaryExpression expression,
		...
		if (offset.numArguments() == 2 && offset.argument(1) instanceof SymbolicExpression) {
			if (offset.operator() == SymbolicOperator.MULTIPLY) {
				SymbolicExpression offsetValue = (SymbolicExpression) offset.argument(1);

				if (offsetValue.argument(0).equals(this.offsetFunction)) {
					// -1 * OFFSET(..., ...)
					SymbolicExpression arg0 = (SymbolicExpression) offset.argument(0);
					...
					if (universe.equals(arg0, universe.integer(-1)).isTrue()) {
						...
						NumericExpression fieldIndex = (NumericExpression) sequence.get(1);
						ReferenceExpression reference = this.symbolicUtil.getSymRef(pointer);

						if (reference.isTupleComponentReference()) {
							int pointerFieldIndex = ((TupleComponentReference) reference).getIndex().getInt();

							if (pointerFieldIndex == this.symbolicUtil.extractInt(null, fieldIndex)) {
								CIVLType type = symbolicUtil.getStaticTypeOfDynamicType(dynamicType);
								SymbolicType expectedType = state
										.getVariableValue(symbolicUtil.getDyscopeId(null, pointer),
												symbolicUtil.getVariableId(null, pointer))
										.type();

								if (type.getDynamicType(universe) == expectedType) {
									return new Evaluation(state, symbolicUtil.setSymRef(pointer, identityReference));
								}
							}
						}
					}
	...
	public Evaluation pointerSubtraction(State state, int pid, String process, BinaryExpression expression,
		...
		if (rightSid == -1 && rightVid == -1) {
			...
			eval = this.dereference(expression.left().getSource(), state, process, null, arrayPtr, false, true);
			state = eval.state;
			array = eval.value;
	...
	private Pair<Evaluation, NumericExpression[]> recomputeArrayIndices(State state, String process, int pointedVid,
		...
		eval = dereference(source, state, process, null, arrayRootPtr, false, true);
		state = eval.state;
	...
	protected Evaluation evaluateMPIContractExpression(State state, int pid, String process,
		...
		try {
			...
			this.errorLogger.logSimpleError(expression.getSource(), state, process,
