...
#define FIRSTCOL(rank) \
((((rank)-((rank)/nprocsx)*nprocsx)*nx)/nprocsx)

#define FIRSTROW(rank) \
((((rank)/nprocsx)*ny)/nprocsy)
...
/*@ requires \valid((double (*)[nxl+2])u + (0 .. (nyl + 1)));
  ...
  @   requires nxl < nx < 4 && nyl < ny < 4;
  @   requires \mpi_agree(nprocsx) && \mpi_agree(nprocsy);
  @   requires \mpi_agree(nx) && \mpi_agree(ny);
  @   requires left == LEFT(rank);
  @   requires right == RIGHT(rank);
  @   requires top == TOP(rank);
  @   requires bottom == BOTTOM(rank);
  @   behavior assign_by_left:
  @     assumes FIRSTCOL(rank)*nprocsx != 0;
