...
void initialize(int np, int nd, vnd_t box, vnd_t *pos, vnd_t *vel, vnd_t *acc) {
  ...
  //srand(4711L);
  int r = 42; // REPLACE RANDOM NUMBER GENERATION
  for (i = 0; i < np; i++) {
    for (j = 0; j < nd; j++) {
      x = (r++) % 10000 / (double)10000.0;
      pos[i][j] = box[j] * x;
      vel[i][j] = 0.0;
      acc[i][j] = 0.0;
    }
...
int main (int argc, char **argv) {
  ...
  //OSCR_init (NUMTHREADS, "Molecular dynamic simulation", "Use md <Nparts> <Nsteps>", NUM_ARGS, 
   // PARAM_NAMES, DEFAULT_VALUES , NUM_TIMERS, NUM_TIMERS, TIMERS_NAMES, 
	...
	NPARTS = NPARTSINIT; //OSCR_getarg_int(1);
  NSTEPS = NSTEPSINIT; //OSCR_getarg_int(2);
