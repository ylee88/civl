...
$input int xProcs;                 // Number of components of columns
$assume xProcs > 1 && xProcs <= XPROCSB;
$input int YPROCSB;                // Bound number of components of rows
$input int yProcs;                 // Number of components of rows
$assume yProcs > 1 && yProcs <= YPROCSB;
$input int _NPROCS;
#else
...
int countRowForProc(int rank) {
  ...
  int b = firstRowForProc(rank+xProcs);
...
int OWNER(int col, int row) {
  int procRow = ((yProcs * (row+1))-1) / ny;
  int procCol = ((col + 1) * xProcs - 1) / nx;

  return procRow * xProcs + procCol;
}
...
void initialization(int argc, char * argv[]) {
  ...
  xProcs = atoi(argv[1]); 
  yProcs = atoi(argv[2]);
  assert(0 < xProcs * yProcs <=nprocs);
  nprocs = (nprocs > xProcs * yProcs)?xProcs*yProcs:nprocs;
  constTemp = 0.0;
  initTemp = 100.0;
  k = 0.13;
...
int main(int argc, char * argv[]) {
  ...
  // elaborating nx, ny, xProcs and yProcs...
  elaborate(nx);
  elaborate(ny);
  elaborate(xProcs);
  elaborate(yProcs);
