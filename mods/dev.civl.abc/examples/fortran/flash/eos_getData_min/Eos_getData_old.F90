subroutine Eos_getData(range,vecLen,solnData,gridDataStruct,eosData,massFrac)
...
#include "Flash.h"
  ...
  real, dimension(:), intent(OUT) :: massFrac
     ...
     do j = jb,je
        do i = ib,ie
           ...
           if(eint_map /= NONEXISTENT) then
              ...
              internalEnergy  = solnData(pres_map,i,j,k) / solnData(dens_map,i,j,k) / (solnData(game_map,i,j,k) - 1.0)
              if(ener_map /= NONEXISTENT) then
                 if ( solnData(ener_map,i,j,k) - kineticEnergy > max(eos_smalle, eos_eintSwitch*kineticEnergy)) then
                    internalEnergy = solnData(ener_map,i,j,k) - kineticEnergy
                 end if
