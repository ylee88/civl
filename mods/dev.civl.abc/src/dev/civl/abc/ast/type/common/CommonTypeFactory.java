...
import dev.civl.abc.ast.type.IF.UnsignedIntegerType;
...
public class CommonTypeFactory implements TypeFactory {
	...
	private FloatingType floatingArithmeticConversion(ArithmeticType type1,
		...
		if (!isFloat1 && !isFloat2) {
			...
			if (kind1 == FloatKind.REAL)
				kind = kind1;
			else if (kind2 == FloatKind.REAL)
				kind = kind2;
			else if (kind1 == FloatKind.LONG_DOUBLE)
				kind = kind1;
	...
	@Override
	public MemType memType(PointerType elementType) {
		return new CommonMemType(elementType);
	}
	...
	@Override
	public SetType theSetType(ObjectType elementType) {
