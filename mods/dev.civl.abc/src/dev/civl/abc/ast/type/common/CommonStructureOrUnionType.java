...
public class CommonStructureOrUnionType extends CommonObjectType
	...
	@Override
	public Iterable<Field> findFieldCycle() {
		Iterable<Field> result = findFieldCycleWorker(new LinkedList<Field>(),
				this);
		return result == null ? new LinkedList<Field>() : result;
	}

	private Iterable<Field> findFieldCycleWorker(LinkedList<Field> fieldStack,
			StructureOrUnionType subType) {
		if (subType.getFields() == null)
			return null;

		if (subType.getFields() == null)
			return null;

		for (Field field : subType.getFields()) {
			if (field != null) {
				StructureOrUnionType fieldType;
				if (field.getType() instanceof ArrayType) {
					ArrayType fieldArrayType = (ArrayType) field.getType();

					while (fieldArrayType
							.getElementType() instanceof ArrayType) {
						fieldArrayType = (ArrayType) fieldArrayType
								.getElementType();
					}

					if (!(fieldArrayType
							.getElementType() instanceof StructureOrUnionType)) {
						continue;
					}

					fieldType = (StructureOrUnionType) fieldArrayType
							.getElementType();
				} else if (field.getType() instanceof StructureOrUnionType) {
					fieldType = (StructureOrUnionType) field.getType();
				} else {
					continue;
				}
				...
				Iterable<Field> childResult = findFieldCycleWorker(fieldStack,
						fieldType);
