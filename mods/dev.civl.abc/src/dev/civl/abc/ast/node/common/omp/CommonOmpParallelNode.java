...
public class CommonOmpParallelNode extends CommonOmpStatementNode
		implements
			OmpParallelNode {
	...
	public CommonOmpParallelNode(Source source, IdentifierNode identifier,
			List<CivlcToken> body, CivlcToken eofToken,
			ExpressionNode numThreads, ExpressionNode ifClause,
			StatementNode statementNode, boolean isDefaultShared) {
	...
	public ASTNode setChild(int index, ASTNode child) {
		if (index >= 10)
			throw new ASTException(
					"CommonOmpParallelNode has ten children, but saw index "
							+ index);
		if (index >= 8 && !(child == null || child instanceof ExpressionNode))
			throw new ASTException("Child of CommonOmpParallelNode at index "
