...
public class ProveTest {
	...
	public void testUninterpretedTypeNCCompare() {
		SymbolicUninterpretedType type = universe.symbolicUninterpretedType("test");
		SymbolicConstant X = universe.symbolicConstant(universe.stringObject("X"), type);
		SymbolicConstant Y = universe.symbolicConstant(universe.stringObject("Y"), type);
		SymbolicConstant Z = universe.symbolicConstant(universe.stringObject("Z"), type);
		BooleanExpression context = universe.and(universe.equals(X, Y), universe.equals(Z, Y));
		BooleanExpression comparison = universe.equals(X, Z);

		universe.setShowProverQueries(true);
		assertEquals(proverFactory.newProver(context).valid(comparison).getResultType(), ResultType.YES);
	}
	...
	public void testUninterpretedTypeNCCompare2() {
		SymbolicUninterpretedType type = universe.symbolicUninterpretedType("test");
		SymbolicExpression k0 = universe.concreteValueOfUninterpretedType(type, universe.intObject(0));
		SymbolicExpression k1 = universe.concreteValueOfUninterpretedType(type, universe.intObject(1));
		SymbolicConstant X = universe.symbolicConstant(universe.stringObject("X"), type);
		BooleanExpression comparison = universe.and(universe.equals(X, k0), universe.equals(X, k1));

		universe.setShowProverQueries(true);
		assertEquals(proverFactory.newProver(universe.trueExpression()).valid(universe.not(comparison)).getResultType(),
				ResultType.YES);
	}
	...
	public void testUninterpretedTypeNCCompare3() {
		...
		SymbolicConstant X = universe.symbolicConstant(universe.stringObject("X"), type);
		SymbolicConstant Y = universe.symbolicConstant(universe.stringObject("Y"), type);
		BooleanExpression context = universe.and(universe.equals(Y, k0), universe.equals(X, k1));
	...
	public void testSummationExpansion() {
		...
		NumericExpression x = (NumericExpression) su.symbolicConstant(su.stringObject("X"), intType);
		NumericExpression y = (NumericExpression) su.symbolicConstant(su.stringObject("Y"), intType);
		SymbolicExpression f = su.symbolicConstant(su.stringObject("f"),
				su.functionType(Arrays.asList(su.integerType()), su.realType()));
		SymbolicConstant i = su.symbolicConstant(su.stringObject("i"), su.integerType());

		f = su.lambda(i, su.apply(f, Arrays.asList(i)));

		NumericExpression f_x_y = (NumericExpression) su.sigma(x, y, f);
		NumericExpression f_x_yPLUS1 = (NumericExpression) su.sigma(x, su.add(y, su.oneInt()), f);
		NumericExpression f_xMINUS1_y = (NumericExpression) su.sigma(su.subtract(x, su.oneInt()), y, f);

		BooleanExpression rightExpansion = su.equals(f_x_yPLUS1,
				su.add(f_x_y, (NumericExpression) su.apply(f, Arrays.asList(y))));
		BooleanExpression leftExpansion = su.equals(f_xMINUS1_y,
				su.add(f_x_y, (NumericExpression) su.apply(f, Arrays.asList(su.subtract(x, su.oneInt())))));
		Reasoner reasoner = su.reasoner(su.lessThanEquals(x, y));

		su.setShowProverQueries(true);
		assertEquals(ResultType.YES, reasoner.valid(leftExpansion).getResultType());
		assertEquals(ResultType.YES, reasoner.valid(rightExpansion).getResultType());
	}
	...
	public void testSummationTransitive() {
		...
		NumericExpression x = (NumericExpression) su.symbolicConstant(su.stringObject("X"), intType);
		NumericExpression y = (NumericExpression) su.symbolicConstant(su.stringObject("Y"), intType);
		NumericExpression z = (NumericExpression) su.symbolicConstant(su.stringObject("Z"), intType);
		SymbolicExpression f = su.symbolicConstant(su.stringObject("f"),
				su.functionType(Arrays.asList(su.integerType()), su.realType()));
		SymbolicConstant i = su.symbolicConstant(su.stringObject("i"), su.integerType());
		...
		ResultType result = su.reasoner(su.and(su.lessThanEquals(x, y), su.lessThanEquals(y, z)))
	...
	public void testProverTranslationForSymbolicTuple() {
		...
		assertEquals("Z3 must be installed for passing this " + "test", true, z3 != null);

		SymbolicUniverse su = SARL.newStandardUniverse(config, z3);
		SymbolicTupleType innerTupleType = su.tupleType(su.stringObject("tuple_inn"), Arrays.asList(su.integerType()));
		SymbolicTupleType tupleType = su.tupleType(su.stringObject("tuple"), Arrays.asList(innerTupleType));
		SymbolicConstant myTuple = su.symbolicConstant(su.stringObject("X"), tupleType);
	 ...
	 * {@link TestConstants#slowNegationFormula(boolean, SymbolicUniverse)} .
	...
	public void testNoUnsatZ3() {
		...
		assertEquals("Z3 must be installed for passing this " + "test", true, z3 != null);
		SymbolicUniverse su = SARL.newStandardUniverse(config, z3);
		BooleanExpression unsatFormula = TestConstants.slowNegationFormula(false, su);
	...
	public void testUnsatSimplify() {
		...
		assertEquals(ResultType.YES, su.reasoner(su.trueExpression()).unsat(unsatFormula).getResultType());
