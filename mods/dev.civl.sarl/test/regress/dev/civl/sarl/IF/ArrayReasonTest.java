...
public class ArrayReasonTest {
	...
	public void setUp() throws Exception {
		...
		intArr_a = universe.symbolicConstant(a_obj, arrayType);
		intArr2D_b = universe.symbolicConstant(b_obj, arrayType2D);
		intArr_c = universe.symbolicConstant(a_obj, arrayType);
		i = (NumericExpression) universe.symbolicConstant(i_obj, integerType);
		j = (NumericExpression) universe.symbolicConstant(j_obj, integerType);
		k = (NumericExpression) universe.symbolicConstant(k_obj, integerType);
		m = (NumericExpression) universe.symbolicConstant(m_obj, integerType);
	}
	...
	public void array2DAccessReason() {
		...
		SymbolicExpression newArray2D = universe.arrayWrite(intArr2D_b, two,
				newRow);
		SymbolicExpression r1 = universe.arrayRead(newArray2D, i);
		SymbolicExpression r2 = universe.arrayRead(r1, j);

		Reasoner r = universe.reasoner(universe.and(universe.equals(i, two),
				universe.equals(j, three)));
	...
	public void arrayCompareReasonTest() {
		...
		Reasoner reasoner2 = universe.reasoner(
				universe.and(universe.lessThan(k, m), universe.neq(i, j)));
	...
	public void arrayWriteReasonTest() {
		...
		for (int i = 0; i < 5; i++) {
			values.add(universe.add((NumericExpression) universe
					.arrayRead(intArr_a, universe.integer(i)), one));
		}
		...
		NumericExpression v2 = (NumericExpression) universe.arrayRead(intArr_a,
				i);
		NumericExpression v1SubV2 = universe.subtract(v1, v2);
		Reasoner reasoner = universe.reasoner(universe.and(
				universe.lessThanEquals(zero, i), universe.lessThan(i, five)));
	...
	/**
	 * This is a test corresponding to the following CIVL-C code:
	 * <pre>
	 * $assume(0 <= t0 <= 1);
	 * int t1 = 1 - t0;
	 * $havoc(&a);
	...
	public void test2() {
		...
		SymbolicArrayType inta = u.arrayType(integerType);
		SymbolicArrayType aType = universe.arrayType(inta, universe.integer(2));
		SymbolicConstant a = universe
				.symbolicConstant(universe.stringObject("a"), aType);
		NumericExpression t0 = (NumericExpression) universe
				.symbolicConstant(universe.stringObject("t0"), integerType);
		NumericExpression t1 = (NumericExpression) universe
				.subtract(universe.integer(1), t0);
		NumericSymbolicConstant i = (NumericSymbolicConstant) universe
				.symbolicConstant(universe.stringObject("i"), integerType);
		// a1 = AW(a, t0, AW(a[t0], i, a[t1][i]))
		SymbolicExpression a1 = u.arrayWrite(a, t0, u.arrayWrite(
				u.arrayRead(a, t0), i, u.arrayRead(u.arrayRead(a, t1), i)));
		SymbolicExpression a2 = u.arrayWrite(a1, t0, u.arrayWrite(
				u.arrayRead(a1, t0), i, u.arrayRead(u.arrayRead(a1, t1), i)));
		BooleanExpression p = universe.and(
				universe.lessThanEquals(universe.integer(0), t0),
				universe.lessThanEquals(t0, universe.integer(1)));
		Reasoner r = u.reasoner(p);
		SymbolicExpression actual = r.simplify(a2);
