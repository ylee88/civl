...
public interface SymbolicUniverse extends CoreUniverse {
	...
	/**
	 * Returns a {@link Reasoner} for the given context. A {@link Reasoner} provides
	 * simplification and reasoning services. The context is the boolean expression
	 * assumed to hold by the reasoner. The Reasoner can be used to determine if a
	 * boolean predicate is valid; it may use an external theorem prover to assist
	 * in this task.
	 * 
	 * @param context the boolean expression assumed to hold by the {@link Reasoner}
	...
	/**
	 * Attempts to extract a concrete numeric value from the given expression, using
	 * the assumption if necessary to simplify the expression. For example, if the
	 * assumption is "N=5" and the expression is "N", this method will probably
	 * return the number 5. If it cannot obtain a concrete value for whatever
	 * reason, it will return null.
	 * 
	 * @param assumption a boolean expression that is assumed to hold
	 * @param expression a symbolic expression of numeric type
	...
	/**
	 * <p>
	 * Apply a default widening operator to the value set references in the given
	 * value set template.
	 * </p>
	 * 
	 * @param context the context (path condition) in which this widen is being
	 *                applied
	 * @param vst     a value set template
	...
	SymbolicExpression valueSetProtectiveWidening(BooleanExpression context, SymbolicExpression vstM,
			SymbolicExpression vstP);

	SymbolicExpression valueSetElimWidening(BooleanExpression context, SymbolicExpression vst,
	 ...
	 * {@link Reasoner}s created by this universe can take use of the definitions of
	 ...
	 * </p>
	 * 
	 * @param logicFunctions an array of {@link ProverFunctionInterpretation}s
	 */
