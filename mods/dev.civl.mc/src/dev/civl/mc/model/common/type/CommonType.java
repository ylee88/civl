...
public abstract class CommonType implements CIVLType {
	...
	/**
	 * CIVL associates a single dynamic type to every CIVL type and does this once
	 * at compile time. All the dynamic types which occur as dynamic types of CIVL
	 * types are numbered from 0. This is used in particular to construct the bundle
	 * type which is the union of all of the dynamic types. This field is the
	 * dynamic type index to this one and it's initially be minus one and can be set
	 * later by calling {@link #setDynamicTypeIndex(int)} and the getter is
	 ...
	 * dynamic type to every CIVL type and does this once at compile time. All the
	 * dynamic types which occur as dynamic types of CIVL types are numbered from 0.
	 * This is used in particular to construct the bundle type which is the union of
	 * all of the dynamic types. This field is the dynamic type index to this one
	 * and it's initially be minus one and can be set later by calling this method
	 * and the getter is {@link #getDynamicTypeIndex()}.
	 * 
	 * @param index the dynamic type index of this CIVL type
	 */
	...
	protected abstract void addFreeVariables(Set<Variable> result, Set<CIVLType> seenTypes);
