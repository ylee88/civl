...
public interface CIVLTypeFactory {
	...
	/**
	 * Gets the CIVL bundle type, which is unique for a given CIVL model. A bundle
	 * type is a union type of all types referenced by a given CIVL model. A bundle
	 * type needs to be completed at the end of the construction of the model.
	 * 
	...
	/**
	 * Returns a new complete array type with specified extent (length expression)
	 * and element type.
	 * 
	 * @param elementType the type of each element in the array
	 * @param extent      the expression of integer type specifying the length of
	 *                    the array
	...
	/**
	 * Creates a complete regular domain type, which is has the given dimension and
	 * is composed by the given range type.
	 * 
	 * @param rangeType the range type
	 * @param dim       the dimension of the domain type
	 * @return the complete regular domain type.
	 */
	CIVLCompleteDomainType completeDomainType(CIVLType rangeType, int dim);

	/**
	 * This returns the universal domain type (<code>$domain</code>). It includes
	 * all the complete domain types (<code>$domain(n)</code>).
	 * 
	 * @param rangeType
	 * 
	 ...
	 * 
	 * @param name     The name of the enumeration type to be created.
	 * @param valueMap The map of enumerator names and their values.
	 * @return The new enumeration type.
	 */
	CIVLEnumType enumType(String name, Map<String, BigInteger> valueMap);

	/**
	 * Creates a new instance of function type, which contains a return type and a
	 * list of parameter types.
	 * 
	 * @param returnType The return type of the function type.
	 * @param paraTypes  The parameter types of the function type.
	 * @return the new function type
	 */
	CIVLFunctionType functionType(CIVLType returnType, CIVLType[] paraTypes);

	/**
	 * Returns a new, incomplete heap type. The heap type must be completed later by
	 * specifying a sequence of malloc statements in method
	 * {@link #completeHeapType}.
	 * 
	 * @param name a name to give to the new heap type
	 * 
	...
	/**
	 * Returns the CIVL heap type, which is unique for a given CIVL model. A heap
	 * type is a struct type of all types appearing in a malloc statement, plus all
	 * handled object types used by the model. A heap type needs to be completed at
	 * the end of the construction of the model.
	 * 
	 ...
	 * 
	 * @param baseType The type pointed to by the pointer.
	 ...
	 * 
	 * @param name          Identifier for the name of this struct member.
	 * @param type          The type of this struct member.
	 * @param index         Index of the field in its enclosing struct/union
	 * @param isAnonymous   Whether this field is an anonymous member
	 * @param enclosingType The struct/union type directly containing this field.
	 * @return A struct field with the given name and type.
	 */
	StructOrUnionField structField(Identifier name, CIVLType type, int index, boolean isAnonymous,
			CIVLStructOrUnionType enclosingType);

	/**
	 * Returns new incomplete struct or union type with given name. Type can be
	 * completed later using one of the "complete" methods in CIVLStructOrUnionType.
	 * 
	 * The struct or union returned is a new instance of struct or union type that
	 * will never be equal to another struct or union type, regardless of identifier
	 * or fields.
	 * 
	 * @param name     identifier, usually the "tag" for this struct or union type
	 * @param isStruct is the new type a struct type? If false, then the new type
	...
	/**
	 * Obtains the CIVL type by the given name. This returns the type that has been
	 * added by {@link #addSystemType(String, CIVLType)}, and returns null if no
	 * such type.
	 * 
	 * @param name The name (key) of the type.
	 * @return the CIVL type of the given name
	 */
	CIVLType systemType(String name);

	/**
	 * Returns the void type. Used in places where a type is required syntactically
	 * but there is no type, such as function which does not return a value.
	 * 
	...
	/**
	 * Returns the {@link CIVLMemType}. A mem type is the type of all expressions
	 * representing a set of pointers.
	 * 
	 * @return the mem type, which is the type of all expressions representing a set
	 *         of pointers
	 */
	CIVLMemType civlMemType();

	/**
	 * Returns the {@link CIVLSetType}. A set type is the type of expressions that
	 * representing a set of objects of a non-set type.
	 * 
	 * @param elementType the element type of the creating set type, note that the
	 ...
	 * 
	 * @return the symbolic type used to represent values of type CIVLDynamicType
	 */
	...
	/**
	 * Add the given type as the object type of one of the heap field. This will add
	 * one more element to the heap type.
	 * 
	 * @param type
	 * @param id
	 */
	 ...
	 * libraries, e.g., $gcomm/$comm for comm, $file for stdio, $gbarrier/$barrier
	 * for concurrency, etc. Each type that will be used by the system library
	 * components (e.g., library executor, etc) should be added explicitly by
	 * calling this method.
	 * 
	 * @param name The name of the type.
	 * @param type The type to be added as a system type
	 */
	void addSystemType(String name, CIVLType type);

	/**
	 * Completes the bundle type by specifying the list of all dynamic types which
	 * can occur as bundle elements. If the collections yields a sequence of types
	 * t_i, then the bundle symbolic type is union_i(array(t_i)).
	 * 
	 * @param bundleType an incomplete bundle type
	 * @param eleTypes   the list of types that could be the element of the bundle
	 *                   type
	 * @param types      the set of all dynamic types which occur as bundle elements
	 */
	 ...
	 * 
	 * @param heapType an incomplete heap type
	 * @param mallocs  sequence of malloc statements that can access heaps of that
	 *                 type
	 */
	void completeHeapType(CIVLHeapType heapType, Collection<MallocStatement> mallocs);

	/**
	 * Returns the type of the heap field of the given index. A CIVL model has its
	 * unique heap type, which has type of tuples of arrays of arrays of type,
	 * determined by malloc statements and handle types appear in the source
	 * program. For examples, given the following program,
	 * 
	 * <pre>
	 * int main(){
	 ...
	 * 
	 * @param type the type which is a field of the heap type
	...
	/**
	 * Return the size of a dynamic type. Note that the given type must not contain
	 * any incomplete array type.
	 * 
	 * @param universe a reference to the {@link SymbolicUniverse}
	 * @param the      a symbolic type
	 * @return the size of a dynamic type
	 */
	NumericExpression sizeofDynamicType(SymbolicType dynamicType);

	/**
	 * Given a symbolic type, returns a canonical symbolic expression which somehow
	 * wraps that type so it can be used as a value. Nothing should be assumed about
	 * the symbolic expression. To extract the type from such an expression, use
	 * method {@link #getType}.
	 * 
	 * @param civlType the CIVL type that the symbolic type corresponds to.
	 * @param type     a symbolic type
	...
	/**
	 * Given a symbolic expression returned by the method {@link #expressionOfType},
	 * this extracts the type that was used to create that expression. If the given
	 * expression is not an expression that was created by
	 * {@link #expressionOfType}, the behavior is undefined.
	 * 
	 * @param expr a symbolic expression returned by method
	...
	/**
	 * Given a symbolic expression returned by the method {@link #expressionOfType},
	 * this returns the {@link CIVLType} which is associated to the dynamic type
	 * that was used to create the given expression. If the given expression is not
	 * an expression that was created by {@link #expressionOfType}, the behavior is
	 * undefined.
	 * 
	 * @param expr a symbolic expression returned by method
	 *             {@link #expressionOfType}
	 * @return the corresponding CIVL type of the given dynamic type which was used
	 *         to create the given expression
	 */
