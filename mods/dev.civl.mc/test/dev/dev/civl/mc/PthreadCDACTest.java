...
public class PthreadCDACTest {
	...
	public void pthread_finding_k_matches() throws ABCException {
		assertTrue(ui.run("verify", "-inputCIVL_argc=3", "-errorBound=2",
				filename("pthread-finding-k-matches.c")));
	}
	...
	public void pthread_jacobi() throws ABCException {
		assertTrue(ui.run("verify", "-inputCIVL_argc=3",
				filename("pthread-jacobi.c")));
	}
	...
	public void pthread_numerical_integration() throws ABCException {
		assertTrue(ui
				.run("verify", filename("pthread-numerical-integration.c")));
	}
