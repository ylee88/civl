...
\subsubsection{Scope comparisons}
...
The pre-defined constant \cscoperoot{} is a reference to the root
dynamic scope.

The operator \texttt{==} may be applied to arguments of type
$\cscope$.  It returns \emph{true} iff the two arguments refer to the
same dynamic scope.

The operator \texttt{>} may be applied to arguments of type $\cscope$.
If returns \emph{true} iff the dynamic scope referred to by the left
argument is a strict ancestor of the dynamic scope referred to by the
right argument, i.e., the left scope contains the right scope.  The
operators \texttt{>=}, \texttt{<}, \texttt{<=} have similar
interpretations.
