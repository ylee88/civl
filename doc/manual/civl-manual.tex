...
%Ideally, there will be two states saved for [Example 2]. The first is the state after executing the first loop, and the second is after executing the second loop.
%
%One question here is, if processes p[0] to p[4] terminates in the order of 0-4, then the loop for(int i = 0; i < 5; i++) $wait p[i]; will have 5 saved states, the transitions of which are like ...p[0] terminates...wait p[0]... p[1] terminates... wait p[1]....
...
%
%If there is only one such process, then that process gets to execute as a normal atomic block, like [Example 2].
%
%[Example 3]
%int x = 1;
%int y = 0;
...
%}
%
%3. a loop in the state space is detected: this means that the atomic block is infinite, we shall throw an exception or report a warning for this.
%
%Here is how to detect the loop.
%- if there is no non-determinism or blocking, then the loop can be detected by maintaining a stack of states at this level.
%
...
%
%We may maintain a counter to record the number of states that have been established in a process's atomic block and report warnings if the counter exceeds some upper bound.
%
%[Example 4]
%int a = 0, b = 0;
%
...
%}
%
%In [Example 4], assume we have the following states during the execution, where s(x,y,z,L) means a state s with a = x, b=y, i=z and location being L.
%
%s1(1,0,0,L3) (after executing a=1)
%s2(1,2,0,L3) (after executing when(2 > 1) b = 2; i++; i--; a=1)
%s3(1,3,0,L3) (after executing when(3 > 1) b = 3; i++; i--; a=1)
%s4(1,2,0,L3) (after executing when(2 > 1) b = 2; i++; i--; a=1)
%s5(1,3,0,L3) (after executing when(3 > 1) b = 3; i++; i--; a=1)
%
%Here s2 and s4 are identical, s5 and s6 are identical. And DfsSearcher should be able to detect the loop because s1, s2, s3 are saved.
